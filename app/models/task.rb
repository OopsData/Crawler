...
class Task
  ...
  #卷入关键词
  ENTANGLE = %w(真实 生活 实验 发展 团结 创造 资源 理想 信任 合作 生存 财富 钱 创新 理念 重建 探索 坚持 规则 淘汰 精神 理解 负责 渴望 回归 自然 提高 直播 参加 报名 劳动 体验 指挥 经济 规划 矛盾 假 没意思 剧本 质疑 作秀 失望 不真实 赞 棒 喜欢 支持 舒服 吐槽 演 挑拨 女朋友 男朋友 夫妇 淫荡 撕逼 )
  ...
  def self.runing_tieba_history_data_tasks
    ...
    tmp_hash.each do |name,hash|
      thread_wrap << Thread.new {
        threads = []
        max_pn  = hash[:max_pn] 
        (0..max_pn).each_slice(5000) do |pn_arr|
          threads << Thread.new {
            spn   = pn_arr.first 
            epn   = pn_arr.last 
  ...
  # 导出贴吧日报表
  def self.import_tieba_day_reports
  end

  # 导出贴吧周报表
  def self.import_tieba_week_reports

  end
  
  # 导出贴吧月报表
  def self.import_tieba_month_reports
  end
  ...
  def self.generate_tieba_keyword_excel(name,from,to,people_kwds=nil,entangle_kws=nil,warn_kwds=nil,program_kwds=nil,story_kwds=nil,disport_kwds=nil,feature_kwds=nil)
    ...
    row_count = 0
    TiebaInfo.where(name:name).each do |t|
      ...
      if date >= from && date <= to
        ...
        rw = [t.name,date,people,entangle,warn,program,story,disport,feature,title,cont,judge_value,reply]
        sheet1.row(row_count + 1).replace(rw)
        row_count += 1         
      end
    end
    book.write Rails.root.to_s + '/public/export/' + "贴吧_#{name}_#{from}_#{to}.xls"
  end
  ...
  def self.generate_tieba_people_excel(name,from,to,people_kwds=nil)
    if people_kwds
      book   = Spreadsheet::Workbook.new  
      sheet1 = book.create_worksheet :name => "#{name}数据"
      sheet1.row(0).concat %w(日期  人物名称  主题量  回帖量)
      TiebaInfo.where(name:name).each do |t|
        date   = t.basic['date'].split(' ').first
        if date >= from && date <= to
  ...
  def self.generate_tieba_increment_excel(name,from,to)
    #from = Date.parse('2010-01-28')
    #to = Date.parse('2010-02-10')
    #from.upto(to) do |d|
    # puts d.strftime('%F')
    #end
    book   = Spreadsheet::Workbook.new 
